...
Parser = (function(PageParser) {
	...
	PageParser.prototype = {
		...
		*/
		...
		parsePages: function(path) {
			...
			for (i = 0, l = fileList.length; i < l; i++) {
				if (fileList[i].match(/\.json$/)) {
					page = this.processPage(path + '/' + fileList[i]);
					if (!page) {
						continue;
					}
		 ...
		 */
		processPage: function(filename) {
			var config,
				page = new Page({
					path: filename,
					filename: filename.replace(PAGE_PATH+'/', '').replace('.json', '')
				});

				console.log("Parse page: ", page.filename, PAGE_PATH);
			try {
				...
				var kids = this.processChildren(page.path.replace('.json', ''));
