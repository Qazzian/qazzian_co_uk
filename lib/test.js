...
var tests = {
	...
	testPageParser: function (){
		...
		childData = parser.processChildren(__dirname + '/../templates/pages/' + 'projects');
		assert.ok(childData, "Have some child data for the projects folder");
		assert.equal(childData[0].filename, 'projects/SiteBuilder', "The crack attack page is defined in the child data");
		// console.log('first child: ', childData[0]);
	},
	...
	testSiteBuilder: function(){
		...
		console.log(JSON.stringify(builder.data));
